...
def _detect_phf_pair(
        prev_lum: np.ndarray, 
        curr_lum: np.ndarray,
        delta_l_threshold=20, 
        min_luminance_threshold=20, 
        area_threshold=0.4) -> dict:
...
class PSEConfig:
    ...
    min_stripe_pairs: int = 5 #이 수치는 건들면 안됨
...
# =========================
def run_cv_metrics(
        video_path: Path, fps: float | None = None,
    ...
    pse_cfg = PSEConfig(
        heavy_every_n=2,                     
        scale_factor=0.65,                    
        gabor_energy_thresh_percentile=99.0,  
