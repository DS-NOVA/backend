...
from app.cruds.interp_client import InterpClient  # [KEEP] IFRNet 로딩 관련 import/경로 등...
...
def adjust_brightness_masked_pil(
    ...
    if print_coverage:
        ...
        print(
            f"[BRIGHT-MASK] coverage={coverage:.2%}, "
            f"thresh={thresh}, atten={atten_factor}, blur={blur_ksize}"
        )
...
async def assemble_interpolated_video(
    video_id: str,
    fps: float,
    base_dir: str,
    frame_ext: str = ".png",
    ...
    print(
        f"[DEBUG][assemble] ramp_params: v_min={ramp_params.v_min}, "
        f"s_min={ramp_params.s_min}, gamma_max={ramp_params.gamma_max}, "
        f"hold_ratio={ramp_params.hold_ratio}"
    )
    ...
    writer = imageio.get_writer(
        ...
        output_params=[
            ...
            "0",
            "-pix_fmt",
            "yuv420p",  # [ADD] 재생 호환성 & 깜빡임 완화
            "-vsync",
            "cfr",  # [ADD] 가변 프레임레이트 흔들림 방지
        ],
    )
    ...
    try:
        ...
        while True:
            ...
            if rgb_prev_raw.shape[:2] != (target_h, target_w):
                rgb_prev_raw = cv2.resize(
                    rgb_prev_raw, (target_w, target_h), interpolation=cv2.INTER_AREA
                )

            # 2) prev 기록 직전 '1회' 램프
            rgb_prev_out = rgb_prev_raw
            if use_vs_ramp and s_smooth[k] > 1e-6:
                rgb_prev_out = _apply_vs_ramp_to_rgb_linear(
                    rgb_prev_out, float(s_smooth[k]), ramp_params
                )
            ...
            if save_intermediate and frame_to_run[k] is not None:
                ...
                Image.fromarray(rgb_prev_out).save(
                    dump_dir / "originals" / f"frame_{k:04d}.png"
                )
            ...
            if insert_mid[k]:
                ...
                if rgb_next_raw.shape[:2] != (target_h, target_w):
                    rgb_next_raw = cv2.resize(
                        rgb_next_raw, (target_w, target_h), interpolation=cv2.INTER_AREA
                    )

                ok0, buf0 = cv2.imencode(".png", cv2.cvtColor(rgb_prev_raw, cv2.COLOR_RGB2BGR))
                ok1, buf1 = cv2.imencode(".png", cv2.cvtColor(rgb_next_raw, cv2.COLOR_RGB2BGR))
                if not (ok0 and ok1):
                    raise RuntimeError("PNG 인코딩 실패")

                # 모델 서버에 중간 프레임 요청 (비동기)
                mid_png_bytes = await interp_client.interpolate_pair(buf0.tobytes(), buf1.tobytes())

                # 받은 PNG를 numpy RGB로 변환
                mid_bgr = cv2.imdecode(np.frombuffer(mid_png_bytes, np.uint8), cv2.IMREAD_COLOR)
                if mid_bgr is None:
                    raise RuntimeError("모델 서버에서 반환된 이미지 디코딩 실패")
                mid_rgb_raw = cv2.cvtColor(mid_bgr, cv2.COLOR_BGR2RGB)
                if mid_rgb_raw.shape[:2] != (target_h, target_w):
                    mid_rgb_raw = cv2.resize(
                        mid_rgb_raw, (target_w, target_h), interpolation=cv2.INTER_CUBIC
                    )
                ...
                if save_intermediate and frame_to_run[k] is not None:
                    Image.fromarray(mid_rgb_out).save(
                        dump_dir / "mids" / f"mid_{k:04d}.png"
                    )
    ...
    return {
        ...
        "inserted": int(sum(insert_mid)),
        "dump_dir": str(dump_dir) if save_intermediate else None,
    }
